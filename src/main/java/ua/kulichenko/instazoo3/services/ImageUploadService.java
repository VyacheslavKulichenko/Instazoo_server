...
import ua.kulichenko.instazoo3.exeption.ImageNotFoundExeption;
...
public class ImageUploadService {
    ...
    public ImageModel uploadImageToUser(MultipartFile file, Principal principal) throws IOException {
        ...
        LOG.info("Uploading image profile to User {}", userinsta.getUsername());

        ImageModel userProfileImage = imageRepository.findByUserId(userinsta.getId()).orElse(null);
        if (!ObjectUtils.isEmpty(userProfileImage)) {
            imageRepository.delete(userProfileImage);
        }
        ImageModel imageModel = new ImageModel();
        imageModel.setUserId(userinsta.getId());
    ...
    public ImageModel getImageToUser(Principal principal) {
        Userinsta userinsta = getUserinstaByPrincipal(principal);
        ImageModel imageModel = imageRepository.findByUserId(userinsta.getId()).orElse(null);
    ...
    public ImageModel getImageToPost(Long postID) {
        ImageModel imageModel = imageRepository.findByPostId(postID)
                .orElseThrow(() -> new ImageNotFoundExeption("Cannot find image to Post: " + postID));
        if (!ObjectUtils.isEmpty(imageModel)) {
            imageModel.setImageBytes(decompressBytes(imageModel.getImageBytes()));
